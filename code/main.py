...
with open('dataset/conversionDic.csv', 'w', encoding="utf-8") as f:
    for key in conversionDic.keys():
        f.write("%s,%s\n" % (key, conversionDic[key]))
'''
...
anime['Name_format'] = anime.apply(lambda row: sub_by_column(row, 'Name'), axis=1)

# genre from categorical to numeric
anime['Genre_format'] = anime.apply(lambda row: sub_by_column(row, 'Genre_format'), axis=1)

# producers from categorical to numeric
anime['Producers_format'] = anime.apply(lambda row: sub_by_column(row, 'Producers_format'), axis=1)

# studios from categorical to numeric
anime['Studios_format'] = anime.apply(lambda row: sub_by_column(row, 'Studios_format'), axis=1)

# type from categorical to numeric
anime['Type_format'] = anime.apply(lambda row: sub_by_column(row, 'Type'), axis=1)
